...
void secondLargestHelper(treeNode *root,int k, int &c)
{
	...
	secondLargestHelper(root->right,k,c);
	//increment the counted nodes
	c++;
	//if c becomes k now, then this is the kth largest element
	if(c==k)
	{
	...
	secondLargestHelper(root->left,k,c);
}
void secondLargest(treeNode *root,int k)
{
	//initialize the count of nodes as 0
	int c=0;
	//c is passed as a reference
	secondLargestHelper(root,k,c);
}
